...
*/
void parser_comp(string s, int vec_clock[], int port_id)
{
    ...
    for(int j = 0; j < s.length(); j++)
    {
        ...
        {
            new_vec_clock[index] = stoi(temp);
            temp = "";
            index++;
            if(index == n)  // one message parsed
            {
                assert(s[j+1] == '\0' || s[j+1] == ':');
                index = 0;
                for(int i = 0; i < n; i++)  // updating current vector clock
                {
                    if(new_vec_clock[i] > vec_clock[i])
                        vec_clock[i] = new_vec_clock[i];
                }
                vec_clock[port_id]++;   // increase vector clock
                pending_messages--;     // decreasing vector clock
                mtx.lock();
                curr_time = time(0);
                local_time = localtime(&curr_time);
                output_VC << "Process" << port_id + 1 << " receives message m" << stoi(sender) + 1 << "_"
                << vec_clock[stoi(sender)] << " from Process" << stoi(sender) + 1
                << " at " << local_time->tm_min << ":" << local_time->tm_sec << " vc: [ ";
                for(int i = 0; i < n; i++)
                   output_VC << vec_clock[i] << " ";
                output_VC << "]" << endl;
                mtx.unlock();
            }
        }
...
void clientRunner(int client_id, vector<int> conns, int vec_clock[], 
                    mutex* node_proc)
{
    ...
    while(completion < n && message_sent < m)
    {
        ...
        if(action_internal && internal_to_do > 0)
        {
            ...
            curr_time = time(0);
            local_time = localtime(&curr_time);
            output_VC << "Process" << client_id + 1 << " executes internal event e" << client_id + 1 << "_"
...
accordingly.
*/
void run_server(int port_id, int vec_clock[], mutex* node_proc)
{
        ...
        {  
            ...
            { 
                ...
                {  
                    ...
                    node_proc->lock();
                    parser_comp(please, vec_clock, port_id);
                    node_proc->unlock();
                }  
            }  
        }  
...
void node(int id)
{
    ...
    thread threads[2];
    threads[0] = thread(run_server, id, logical_time, &node_proc);
    threads[1] = thread(clientRunner, id, conns, logical_time, &node_proc);
...
int main()
{
    srand(time(0));
    ifstream infile;
    output_VC.open("output-vc.txt", ios::out | ios::trunc);
    ...
    out_compare << "The input space used by normal vc is : O(" << n << ")." << endl;
    out_compare << "The space used for sending message in normal vc is : O(" << n*n*m << ")." << endl;
    return 0;
}
